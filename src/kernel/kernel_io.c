...
static const kernel_io_external_pin_t external_pin_table[kernel_shared_direction_channel_last+1] = {
   ...
   { PCAL6116A_PORTX, 0xff, 0xff, 0 }, //IO7
...
int set_io_function(const toolbox_io_attr_t * attributes){
   ...
   if( attributes->peripheral_function == CORE_PERIPH_I2C ){
      ...
      switch(attributes->peripheral_port){
         case 0:
            result |= acquire_pin(CORE_PERIPH_I2C, kernel_shared_direction_channel1, IN);
            result |= acquire_pin(CORE_PERIPH_I2C, kernel_shared_direction_channel2, IN);
            if( result == 0 ){
         ...
         case 1:
            result |= acquire_pin(CORE_PERIPH_I2C, kernel_shared_direction_channel3, IN);
            result |= acquire_pin(CORE_PERIPH_I2C, kernel_shared_direction_channel4, IN);
            if( result == 0 ){
   ...
   if( attributes->peripheral_function == CORE_PERIPH_SPI ){
      ...
      if( attributes->o_flags & TOOLBOX_IO_IS_INVERT_DIRECTION ){
         //slave
         result |= acquire_pin(CORE_PERIPH_SPI, kernel_shared_direction_channel3, IN);
         result |= acquire_pin(CORE_PERIPH_SPI, kernel_shared_direction_channel4, IN);
         result |= acquire_pin(CORE_PERIPH_SPI, kernel_shared_direction_channel5, OUT);
         result |= acquire_pin(CORE_PERIPH_SPI, kernel_shared_direction_channel6, IN);
         if( result == 0 ){
            set_pin_direction(kernel_shared_direction_channel3, IN);
            set_pin_direction(kernel_shared_direction_channel4, IN);
         ...
         //master
         result |= acquire_pin(CORE_PERIPH_SPI, kernel_shared_direction_channel3, OUT);
         result |= acquire_pin(CORE_PERIPH_SPI, kernel_shared_direction_channel4, OUT);
         result |= acquire_pin(CORE_PERIPH_SPI, kernel_shared_direction_channel5, IN);
         result |= acquire_pin(CORE_PERIPH_SPI, kernel_shared_direction_channel6, OUT);
         if( result == 0 ){
   ...
   if( attributes->peripheral_function == CORE_PERIPH_I2S ){
      if( attributes->o_flags & TOOLBOX_IO_IS_INVERT_DIRECTION ){
         //slave
         result |= acquire_pin(CORE_PERIPH_I2S, kernel_shared_direction_channel9, IN);
         result |= acquire_pin(CORE_PERIPH_I2S, kernel_shared_direction_channel10, IN);
         result |= acquire_pin(CORE_PERIPH_I2S, kernel_shared_direction_channel11, IN);
         result |= acquire_pin(CORE_PERIPH_I2S, kernel_shared_direction_channel12, OUT);
         if( result == 0 ){
         ...
         //master
         result |= acquire_pin(CORE_PERIPH_I2S, kernel_shared_direction_channel9, IN);
         result |= acquire_pin(CORE_PERIPH_I2S, kernel_shared_direction_channel10, OUT);
         result |= acquire_pin(CORE_PERIPH_I2S, kernel_shared_direction_channel11, OUT);
         result |= acquire_pin(CORE_PERIPH_I2S, kernel_shared_direction_channel12, OUT);
         if( result == 0 ){
            set_pin_direction(kernel_shared_direction_channel9, IN);
...
int init_external_pins(){
   for(u32 i=0; i < kernel_shared_direction_channel_last+1; i++){
      int result;
      ...
      if( io_pin->expansion_port != PCAL6116A_PORTX ){
         ...
         attributes.o_flags = io_pin->init_state;
